...
import { BacktestService, BacktestResult, CompareResult, HistoryRun, MonteCarloResponse } from './backtest.service';
...
export class App implements OnInit {
  ...
  // canvas for the Risk Analytics / Monte Carlo fan chart
  @ViewChild('fanChart') fanChartCanvas?: ElementRef<HTMLCanvasElement>;

  activeTab: 'run' | 'compare' | 'history' | 'risk' = 'run';
  ...
  // Risk Analytics / Monte Carlo tab state -- separate from the other
  // tabs' state for the same reason Compare's state is separate: each
  // tab runs independently and switching away shouldn't clear results
  mcTicker = '';
  mcDays = 252;
  mcNumSimulations = 1000;
  mcTargetPrice: number | null = null;
  mcResult: MonteCarloResponse | null = null;
  mcLoading = false;
  mcErrorMessage = '';
  ...
  private mcFanChart: Chart | null = null;
  ...
  setTab(tab: 'run' | 'compare' | 'history' | 'risk'): void {
    this.activeTab = tab;
    ...
    if (tab === 'risk' && this.mcResult) {
      this.cdr.detectChanges();
      requestAnimationFrame(() => this.drawFanChart());
    }
    if (tab === 'history') {
      this.loadHistory();
    }
  ...
  runMonteCarlo(): void {
    if (!this.mcTicker) return;

    this.mcLoading = true;
    this.mcErrorMessage = '';
    this.mcResult = null;
    this.mcFanChart?.destroy();
    this.mcFanChart = null;

    this.backtestService.runMonteCarlo({
      ticker: this.mcTicker,
      days: this.mcDays,
      num_simulations: this.mcNumSimulations,
      target_price: this.mcTargetPrice ?? undefined
    }).subscribe({
      next: (res: MonteCarloResponse) => {
        this.mcResult = res;
        this.mcLoading = false;
        // same DOM-not-ready-yet fix already used for the compare chart
        this.cdr.detectChanges();
        requestAnimationFrame(() => this.drawFanChart());
      },
      error: (err: any) => {
        this.mcErrorMessage = err?.error?.error || 'Something went wrong running the simulation.';
        this.mcLoading = false;
      }
    });
  }

  private drawFanChart(): void {
    if (!this.mcResult || !this.fanChartCanvas) return;

    const paths = this.mcResult.chart_paths; // [day][simulation]
    const numDays = paths.length;
    const labels = Array.from({ length: numDays }, (_, i) => i.toString());
    const numPaths = paths[0]?.length ?? 0;

    // one translucent dataset per simulated path -- overlapping lines form
    // the fan shape. legend/tooltip off since ~100 dataset entries would
    // be unreadable either way
    const datasets = [];
    for (let simIndex = 0; simIndex < numPaths; simIndex++) {
      datasets.push({
        data: paths.map(dayRow => dayRow[simIndex]),
    ...
    this.mcFanChart = new Chart(this.fanChartCanvas.nativeElement, {
      type: 'line',
      data: { labels, datasets },
      options: {
        responsive: true,
